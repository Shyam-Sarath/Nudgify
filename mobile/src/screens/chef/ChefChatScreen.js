...
export default function ChefChatScreen({ route, navigation }) {
  ...
  const currentChefId = user?.id;

  useEffect(() => {
    if (currentChefId) {
      socketService.connect(currentChefId);

      const handleNewMessage = (msg) => {
        if ((msg.sender_id === customerId && msg.receiver_id === currentChefId) ||
            (msg.sender_id === currentChefId && msg.receiver_id === customerId)) {
  ...
  const fetchMessages = async () => {
    try {
      const res = await apiClient.get(`/api/chat/${customerId}`);
      setMessages(res.data.data || []);
      // Mark read
      await apiClient.put(`/api/chat/${customerId}/read`);
    } catch (err) {
  ...
  const handleSend = () => {
    ...
    socketService.sendMessage(customerId, inputText.trim());
    setInputText('');
  };

  const renderMessage = ({ item }) => {
    const isMe = item.sender_id === currentChefId;
    return (
      <View style={[
...
const styles = StyleSheet.create({
  ...
  messageBubble: { maxWidth: '80%', padding: 12, marginBottom: 12 },
  ...
  emptyText: { textAlign: 'center', paddingHorizontal: 32, lineHeight: 22 },
  inputContainer: {
    flexDirection: 'row',
    alignItems: 'flex-end',
    paddingHorizontal: 16,
